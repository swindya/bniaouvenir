...
function RemoveItem(kodename) {
	document.forms.BarangEntry.data.value = localStorage.removeItem(kodename);
	doShowAll();
}
		...
		for (i = 1; i <= localStorage.length - 1; i++) {
			...
			if (i%2 === 0)
			{
				list += '<tr height="30px" STYLE="background-color: #FCFCCC;"><td STYLE="text-align: center; width: 50px;">' + (i) + '</td>\n<td STYLE="text-align: center; width: 100px;">' + key + 
					...
					+ ourArray[2] + '</td>\n<td STYLE="text-align: center; width: 100px">' 
					+ ourArray[3] + '</td>\n<td STYLE="text-align: center; width: 100px">' +
					'<font face="arial" size="1" color="black"><a href="#popup" class="small button blue" onclick="filleditdata(<?php echo $datain;?>); return true;">Edit</a></font>' 
